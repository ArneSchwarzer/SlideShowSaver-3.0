// PartikelComputeShaderAPC.hlsl
//
// V0.4
//
// GPU-Partikelsimulation mit Brandkanten-Emitter.
// Jetzt auch mit effizientem Active Particle Compaction (APC)
//
// Flugbahn:
...
AppendStructuredBuffer<uint> RenderPartikelIndices : register(u1);
...
void CSMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    // --------------------------------------------------------
        ...
        // --------------------------------------------------------
            ...
            /*
             * Wenn der tote Slot in diesem Frame erfolgreich
             * wiedergeboren wurde, gehört er sofort in die
             * APC-Renderliste.
             */

            if (particle.lifetime > 0.0)
            {
                RenderPartikelIndices.Append(particleIndex);
            }
...
// --------------------------------------------------------
    ...
    /*
     * Bis hierher gelangen ausschließlich Partikel,
     * die weder an Alter noch durch Verlassen des
     * sichtbaren Bereichs gestorben sind.
     */
